...
public class IntHistogram implements Histogram<Integer>{
  ...
  public float computeReductionFactor(PredicateOperator predicate,
    ...
    switch (predicate) {
      ...
      case LESS_THAN_EQUALS:
        return (float)1/getNumDistinct() + (float)(value.getInt()-getMinValue())/(getMaxValue()-getMinValue());
      case GREATER_THAN:
        return (float)(getMaxValue()-value.getInt())/(getMaxValue()-getMinValue());
      case GREATER_THAN_EQUALS:
        return (float)1/getNumDistinct() +(float)(getMaxValue()-value.getInt())/(getMaxValue()-getMinValue());
      default:
        return (float)1/getNumDistinct();
    }
  }
