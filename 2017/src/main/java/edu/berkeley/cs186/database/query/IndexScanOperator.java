...
public class IndexScanOperator extends QueryOperator {
  ...
  public int estimateIOCost() throws QueryPlanException {
    /* TODO: Implement me! */
    //Cost = (NPages(T)	+ NTuples(R)) * [prod. of RFs]
    try {
      int pgs = this.transaction.getNumIndexPages(tableName, columnName);
      long recs = transaction.getNumRecords(tableName);
      float rf = this.transaction.getStats(this.tableName).getReductionFactor(this.columnIndex, this.predicate, this.value);
      return (int) Math.ceil((double) (pgs + recs)*rf);
