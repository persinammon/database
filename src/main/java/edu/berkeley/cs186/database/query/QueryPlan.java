...
public class QueryPlan {
  ...
  private QueryOperator pushDownSelects(QueryOperator source, int except) throws QueryPlanException, DatabaseException {
    /* TODO: Implement me! */
      ...
      if (except != -1 && !selectOperators.isEmpty()) {
        selectColumnNames.remove(except);
        selectDataBoxes.remove(except);
        selectOperators.remove(except);
      }
      try {
        addSelects();
  ...
  private QueryOperator minCostJoinType(QueryOperator leftOp,
    ...
    QueryOperator temp = new BNLJOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
    if(temp.estimateIOCost() < minOp.estimateIOCost()) minOp = temp;
    temp = new BNLJOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
    if(temp.estimateIOCost() < minOp.estimateIOCost()) minOp = temp;
    temp = new PNLJOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
    if(temp.estimateIOCost() < minOp.estimateIOCost()) minOp = temp;
    temp = new GraceHashOperator(leftOp, rightOp, leftColumn, rightColumn, transaction);
    if(temp.estimateIOCost() < minOp.estimateIOCost()) minOp = temp;
    return minOp;
  }
  ...
  private Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
    ...
    Map<Set, QueryOperator> map = new HashMap<Set, QueryOperator>();
    /* TODO: Implement me! */
    Iterator<Map.Entry<Set, QueryOperator>> iter = prevMap.entrySet().iterator();
    while (iter.hasNext()) { //each table set
      Map.Entry<Set, QueryOperator> entry = iter.next();
      QueryOperator currentop = null;
      int save = 0;
      for (int i = 0; i<joinTableNames.size(); i++) { //each join predicate
        if (!entry.getKey().contains(joinTableNames.get(i))) { //not already joined lol - one in and one out, do i get a list
          String left_table = joinLeftColumnNames.get(i).split("[.]")[0];
          if (entry.getKey().contains(left_table)) {
            Set<String> rightOpkey = new HashSet<String>();
            rightOpkey.add(joinTableNames.get(i));
            QueryOperator newop = minCostJoinType(entry.getValue(), pass1Map.get(rightOpkey), joinLeftColumnNames.get(i), joinRightColumnNames.get(i));
            if (currentop == null || newop.estimateIOCost() < currentop.estimateIOCost()) {
              currentop = newop;
              save = i;
            }
          }
      ...
      if (currentop != null) {
        ...
        key.add(joinTableNames.get(save));
        map.put(key, currentop);
      }
    }
    return map;
  }
